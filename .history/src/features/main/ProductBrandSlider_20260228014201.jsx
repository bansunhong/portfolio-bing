...
export default function ProductBrandSlider() {
    ...
    useEffect(() => {
        ...
        const onScroll = () => {
            if (window.scrollY > lastY) scrollDir.current = "down";
            else scrollDir.current = "up";
        ...
        const observer = new IntersectionObserver(
            ([entry]) => {
                ...
                if (scrollDir.current === "down") {
                    if (entry.target === productRef.current && !productPlayed.current) {
                        setProductVisible(true);
                        productPlayed.current = true;
                    }
                    if (entry.target === brandRef.current && !brandPlayed.current) {
                        setBrandVisible(true);
                        brandPlayed.current = true;
                    }
                }
                // 위로 올릴 때는 아무 작업도 안 함 → 애니메이션 절대 안 나옴
            },
            { threshold: 0.85 },
        );
