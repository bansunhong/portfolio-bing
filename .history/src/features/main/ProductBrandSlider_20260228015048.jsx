        ...
        const observer = new IntersectionObserver(
            ([entry]) => {
                ...
                if (entry.target === productRef.current) {
                    if (scrollDir.current === "down" || !productPlayed.current) {
                        setProductVisible(true); // 애니메이션 보여줌
                        productPlayed.current = true; // 기록
                    } else {
                        setProductVisible(false); // 올릴 때는 애니메이션 없이 그냥 보이도록 false
                    }
                }

                if (entry.target === brandRef.current) {
                    if (scrollDir.current === "down" || !brandPlayed.current) {
