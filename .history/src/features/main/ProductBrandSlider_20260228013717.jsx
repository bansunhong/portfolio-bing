...
export default function ProductBrandSlider() {
    ...
    // 스크롤 방향 감지
    const productAnimated = useRef(false);
    const brandAnimated = useRef(false);
        ...
        const observer = new IntersectionObserver(
            ([entry]) => {
                ...
                if (entry.target === productRef.current) {
                    if (!productAnimated.current) {
                        setProductVisible(true);
                        productAnimated.current = true; // 한번 실행 후 기억
                    }
                }

                // ✅ BRAND
                if (entry.target === brandRef.current) {
                    if (!brandAnimated.current) {
                        setBrandVisible(true);
                        brandAnimated.current = true; // 한번 실행 후 기억
                    }
                }
            },
            { threshold: 0.85 },
