...
export default function ProductBrandSlider() {
    ...
    // ref로 상태 유지
    const productPlayed = useRef(false);
    ...
    useEffect(() => {
        ...
        const onScroll = () => {
            scrollDir.current = window.scrollY > lastY ? "down" : "up";
        ...
        const observer = new IntersectionObserver(
            ([entry]) => {
                ...
                if (entry.target === productRef.current) {
                    if (!productPlayed.current || scrollDir.current === "down") {
                        setProductVisible(false);
                        setTimeout(() => setProductVisible(true), 50);
                        productPlayed.current = true;
                    }
                }

                if (entry.target === brandRef.current) {
                    if (!brandPlayed.current || scrollDir.current === "down") {
                        setBrandVisible(false);
                        setTimeout(() => setBrandVisible(true), 50);
            ...
            <div ref={productRef} className={styles.sectionProductBlock}>
                ...
                <Slider {...productSettings} className={styles.sliderList}>
                    {productList.map((item, idx) => (
                        <div key={idx}>
                            <div className={`${styles.sliderItem} ${productVisible ? styles.show : ""}`} style={{ animationDelay: `${idx * 0.3}s` }}>
